...
label's review array. That position carries real ordering rather than being
...
reviewers who joined the platform later occupy later positions
($\rho = 0.29$, $p < 10^{-26}$, using label creation order as a proxy for
arrival). We therefore treat position as sequence.

Given that, we can compare a label's first three reviews against its later ones
...
%   - DONE vote_index validated as a real sequence (permutation test z=195,
%     alphabetical ordering ruled out, later-joining reviewers sit later).
%     Worth still confirming with the maintainers that it is specifically
%     chronological rather than insertion order, though they coincide.
