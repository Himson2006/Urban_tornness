...
The variance decomposition reverses between the two outcomes
(Table~\ref{tab:variance}): disagreement is predominantly a property of the
label, whereas uncertainty is predominantly a property of the reviewer.

\begin{table}[tb]\centering
\caption{Variance in individual review decisions attributable to the label
versus the reviewer. Splitting into camps is largely about the sidewalk;
answering ``not sure'' is largely about the person.}
\label{tab:variance}
\begin{tabular}{lcc}
\hline
\textbf{Source} & \textbf{Disagree} & \textbf{Unsure} \\
\hline
Label     & \textbf{31.2\%} & 14.3\% \\
Reviewer  & 9.5\%  & \textbf{20.8\%} \\
Residual  & 59.3\% & 64.9\% \\
\hline
\end{tabular}
\end{table}
...
which labels attract additional review. Restricting to the 21{,}042 labels with
at least six reviews, the naive comparison suggests that uncertainty declines
over a label's lifetime while disagreement grows.

That first impression is wrong, and instructively so. Early reviewers are
themselves more unsure-prone than later ones (mean reviewer effect $+0.043$
versus $+0.020$). Subtracting each reviewer's estimated tendency before
comparing reverses the uncertainty result while leaving the disagreement result
intact (Table~\ref{tab:resolve}). Neither form of disagreement resolves with
additional review; both worsen slightly. Effect sizes are small against base
rates of 0.08 and 0.31, but the direction is unambiguous at this sample size.

\begin{table}[tb]\centering
\caption{Within-label comparison of a label's first three reviews against its
later ones ($n=21{,}042$ labels). Adjusting for reviewer tendency reverses the
apparent resolution of uncertainty.}
\label{tab:resolve}
\begin{tabular}{llccc}
\hline
\textbf{Outcome} & \textbf{Adjusted} & \textbf{Early} & \textbf{Later} & \textbf{$\Delta$} \\
\hline
Unsure   & no  & 0.106 & 0.096 & $-0.009$ \\
Unsure   & yes & 0.082 & 0.090 & $+0.008$ \\
Disagree & no  & 0.308 & 0.337 & $+0.030$ \\
Disagree & yes & 0.313 & 0.340 & $+0.026$ \\
\hline
\end{tabular}
\end{table}
...
%   - per-city breakdown, to show the pattern is not driven by one deployment
%   - bootstrap CIs on the QC retention rates
