...
ambiguous middle; \emph{full}, all reviewed labels; and \emph{soft}, all labels
with the agreement rate as a continuous target.
...
soft      & 0.905 & 0.085 & 0.469 & \textbf{12.9\%} \\
\bottomrule
\end{tabular}
\end{table}
...
Moving from hard to soft targets reduces silent assertions further, to 12.9\%.
We report this but weight it less: that model is a regressor, so part of the
reduction is mechanical rather than learned. The comparison we rely on is
\emph{filtered} against \emph{full\_sub}, where both models are classifiers
trained on equal amounts of data.
